...
// TestCertificateMsgTLS13PreservesOriginalBytes locks in the fix for the
// transcript divergence: certificateMsgTLS13 must feed transcriptMsg the exact
// bytes it was unmarshaled from, not a (potentially lossy) re-marshal.
func TestCertificateMsgTLS13PreservesOriginalBytes(t *testing.T) {
	...
	// It must expose its original wire bytes for the transcript.
